...
function M.next()
	...
	for _, marker in pairs(buf_cache.positions) do
		...
		if
			(dist > 0 and min_positive_dist == nil)
			or (min_positive_dist ~= nil and dist < min_positive_dist and dist > 0)
		then
...
function M.previous()
	...
	for _, marker in pairs(buf_cache.positions) do
		dist = marker.delimiter_lnum - current_lnum
		if
			(dist < 0 and max_negative_dist == nil)
			or (max_negative_dist ~= nil and dist > max_negative_dist and dist < 0)
		then
